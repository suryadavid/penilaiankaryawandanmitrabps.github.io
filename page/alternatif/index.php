...
if (isset($_GET['ta'])) {
  ...
  echo '';
	...
	if (isset($_POST['simpan'])) {
      ...
      $con->query($sql);
      $last_id = $con->insert_id;
      $dos="UPDATE mitra SET status='ok' where nip='$_POST[NIP]'";
      $con->query($dos);
          ...
          <div class="box-body">
            <div class="form-group">
              ...
              <select name="NIP" id="" class="form-control" required>
                <option disabled selected>-- Pilih Alternatif --</option>
                <?php 
                //$sql = "select * from mitra where status='$_GET[d]'";
                ...
                $query = mysqli_query($con, $sql);
                while ($row = mysqli_fetch_assoc($query)) {
                  echo "<option value=$row[NIP]>$row[NIP] - $row[nama_mitra]</option>";
                }
               ?>
              </select>
            </div>
...
<div class="col-lg-4" hidden>
        ...
        <center><label>-- Atau --</label></center>
       <!--  <a href="index.php?p=alternatif&act=upload" class="btn btn-success" style="width: 100%"><i class="fa fa-upload"></i> Upload Nilai via Excel</a> -->
        <center><h2>Upload Data mitra<br><small>Import via Excel</small></h2></center>
